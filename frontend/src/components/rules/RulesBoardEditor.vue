...
const addItemLane = ref<ChecklistLaneKey>('before')
...
<template>
  <section class="panel rules-builder-shell">
    ...
    <div v-else class="rules-builder-empty">
      <p class="section-title">Select a rule set</p>
...
.rules-lane-head {
  ...
  min-height: 4.1rem;
  padding: 0 1rem;
  background: color-mix(in srgb, var(--panel-strong) 84%, var(--panel) 16%);
  border-bottom: 1px solid color-mix(in srgb, var(--border) 14%, transparent 86%);
}
...
.rules-lane-add :deep(svg) {
  width: 1.05rem;
  height: 1.05rem;
}

.rules-lane-add:hover {
  color: var(--text);
}
